...
def generate_full_cetis_report(meta_info, stats_results, ec_results, raw_df, fig, report_type="full"):
    ...
    <body>
        ...
        <table class="info-table">
            <tr><td><b>Test Name:</b> {test_name}</td><td><b>Date:</b> {now}</td></tr>
            <tr><td><b>Endpoint:</b> {endpoint_label}</td><td><b>Method:</b> Optimal Pro Ver.</td></tr>
        </table>
        {comparison_html}
...
def calculate_ec_lc_range(df, endpoint_col, control_mean, label, is_animal_test=False):
    ...
    method, plot_info = "Linear Interpolation (ICp)", {}
    
    try:
        # Probit Only for Animals
        ...
        df_icp = df.copy().rename(columns={df.columns[0]:'Concentration'})
    ...
    if st.button("상세 분석 실행"):
        ...
        for i, t in enumerate(times):
            with tabs[i]:
                ...
                idx = res['p'].index(50)
                st.metric(f"EC50", f"**{res['value'][idx]}**", f"95% CI: {res['95% CI'][idx]}")
                st.dataframe(pd.DataFrame(res))
                fig = plot_ec_lc_curve(pi, f"{t} EC", res, "Immobility (%)")
                
                meta = {'test_type': 'Daphnia Acute', 'protocol': 'OECD TG 202', 'species': 'Daphnia magna', 
                        'endpoint': 'Immobility', 'method_ec': met, 'is_animal': True, 'total_n': df['총 개체수'].mean(), 'col_name': 'Response'}
                raw_renamed = df.rename(columns={'농도(mg/L)':'Concentration', col:'Response'})
                html = generate_full_cetis_report(meta, None, res, raw_renamed, fig, "simple")
    ...
    if st.button("상세 분석 실행"):
        ...
        for i, t in enumerate(times):
            with tabs[i]:
                ...
                idx = res['p'].index(50)
                st.metric(f"LC50", f"**{res['value'][idx]}**", f"95% CI: {res['95% CI'][idx]}")
                
                if t=='96h' and 'Probit' in met:
                    st.info(f"📐 Slope: {pi.get('slope',0):.4f}")

                st.dataframe(pd.DataFrame(res))
                fig = plot_ec_lc_curve(pi, f"{t} LC", res, "Lethality (%)")
                
                meta = {'test_type': 'Fish Acute', 'protocol': 'OECD TG 203', 'species': 'Fish', 
                        'endpoint': 'Lethality', 'method_ec': met, 'is_animal': True, 'total_n': df['총 개체수'].mean(), 'col_name': 'Response'}
                raw_renamed = df.rename(columns={'농도(mg/L)':'Concentration', col:'Response'})
                html = generate_full_cetis_report(meta, None, res, raw_renamed, fig, "simple")
