...
# -----------------------------------------------------------------------------
def generate_full_cetis_report(meta_info, stats_results, ec_results, raw_df, summary_df, dose_resp_fig, raw_resp_fig=None, qq_fig=None, growth_fig=None, report_type="full"):
    # 이미지 변환
    ...
    if st.button("상세 분석 실행"):
        ...
        for i, t in enumerate(times):
            with tabs[i]:
                ...
                # 동물 보고서 생성 (안전 호출)
                html = generate_full_cetis_report(meta, None, res, df_detail, summ.rename(columns={'농도(mg/L)':'Concentration', col:'Response'}), fig, report_type="simple")
