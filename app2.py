...
st.markdown("""
이 앱은 **OECD TG / CETIS Flowchart** 기준을 엄격히 준수합니다.
1. **조류 (Continuous):** Linear Interpolation (ICPIN) 자동 적용.
2. **동물 (Quantal):** Log-Normal 확인 후 Probit(Yes) 또는 TSK(No) 자동 분기.
""")
...
def generate_full_cetis_report(meta_info, stats_results, ec_results, detail_df, summary_df, dose_resp_fig, raw_resp_fig=None, qq_fig=None, growth_fig=None, report_type="full"):
    ...
    try:
        ...
        c_col = 'Concentration'
        v_cols = [c for c in df_detail.columns if c != 'Concentration']
        v_col = v_cols[0]
...
def calculate_tsk(df, endpoint_col):
    ...
    x = np.log10(df_agg['농도(mg/L)'].replace(0, np.nan).dropna())
    p = p.iloc[1:].values; x = x.values; n = df_agg['총 개체수'].iloc[1:].values
    
    # Simple Spearman-Karber
    sum_val = 0
    for i in range(len(p)-1): sum_val += (p[i+1] - p[i]) * (x[i] + x[i+1])
    log_lc50 = sum_val / 2
    lc50 = 10**log_lc50
    ...
    for i in range(1, len(p)-1):
        qi = 1 - p[i]
        if n[i] > 0: var_sum += (p[i] * qi * (x[i+1] - x[i-1])**2) / (4 * n[i])
    se = np.sqrt(var_sum)
    return lc50, f"({10**(log_lc50 - 1.96*se):.4f} ~ {10**(log_lc50 + 1.96*se):.4f})"

# -----------------------------------------------------------------------------
# [함수 2] ICPIN + Bootstrap
# -----------------------------------------------------------------------------
...
def calculate_ec_lc_range(df, endpoint_col, control_mean, label, is_animal_test=False):
    ...
    # 1. Quantal Data? -> YES (Animal) / NO (Algae)
    if is_animal_test:
        # 2. Log-Normal? (Check Probit Fit)
        try:
            # Try Probit
            ...
            if slope <= 0: raise ValueError # 기울기 음수면 불가
            
            # R2 Check (0.95 Strict for Log-Normal assumption)
            pred = model.predict()
            actual = grouped['Response']/grouped['Total']
            r_sq = np.corrcoef(actual, pred)[0,1]**2 if len(actual)>1 else 0
            
            if r_sq < 0.95: raise ValueError("Not Log-Normal")
            ...
            return ec_res, 0, "GLM Probit Analysis", {'type':'probit', 'x': grouped['Log_Conc'], 'y': stats.norm.ppf(grouped['Response']/grouped['Total']), 'slope':slope, 'intercept':intercept, 'x_original': grouped['농도(mg/L)'], 'y_original': grouped['Response']/grouped['Total']}

        except:
            # 3. Log-Normal NO -> Try TSK (if monotonic)
             try:
                 # Monotonic check
                 if dose_resp['Inhibition'].is_monotonic_increasing and dose_resp['Inhibition'].max() >= 0.5:
                    ...
                    return ec_res, 0, "Trimmed Spearman-Karber (TSK)", {'type':'linear', 'x_original': dose_resp['농도(mg/L)'], 'y_original': dose_resp['Inhibition']}
             except: pass
             
             # 4. TSK NO -> ICPIN
             pass # Go to ICPIN block below

    # ICPIN Block (Algae or Fallback Animal)
    ...
    return ec_res, 0, "Linear Interpolation (ICPIN/Bootstrap)", plot_info
