...
2. **검증:** **NOEC/LOEC 산출 로직 전면 수정 (Zero-Variance 대응)**.
""")
...
def generate_full_cetis_report(meta_info, stats_results, ec_results, detail_df, summary_df, dose_resp_fig, raw_resp_fig=None, qq_fig=None, growth_fig=None, report_type="full"):
    ...
    for _, row in summary_df.iterrows():
        ...
        ci_min = m - 1.96 * se; ci_max = m + 1.96 * se
        ...
        summ_rows += f"""<tr><td>{row[conc_col]}</td><td>{n}</td><td>{m:.4f}</td><td>{ci_min:.4f}</td><td>{ci_max:.4f}</td><td>{row['min']:.4f}</td><td>{row['max']:.4f}</td><td>{se:.4f}</td><td>{cv:.2f}%</td><td>{effect:.2f}%</td></tr>"""
    ...
    try:
        ...
        for conc, row in pivot_df.iterrows():
            if meta_info.get('is_animal', False): vals = "".join([f"<td>{int(v)}</td>" for v in row])
            else: vals = "".join([f"<td>{v:.4f}</td>" for v in row])
    ...
    comp_detail_html = ""
    if 'comp_details' in stats_results and stats_results['comp_details']:
        comp_rows = ""
        ...
        comp_detail_html = f"""<div class="section-title">4. Detailed Comparison Results (vs Control)</div><table><tr><th>Concentration</th><th>T-Stat</th><th>P-Value</th><th>Alpha</th><th>Result</th></tr>{comp_rows}</table>"""
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="utf-8"><style>@page {{ size: A4; margin: 15mm; }} body {{ font-family: 'Arial', 'Malgun Gothic', sans-serif; font-size: 10pt; color: #000; line-height: 1.3; }} .header-box {{ border: 2px solid #000; padding: 10px; margin-bottom: 10px; background-color: #f9f9f9; text-align: center; }} .header-title {{ font-weight: bold; font-size: 16pt; }} .section-title {{ font-weight: bold; font-size: 11pt; background-color: #e6e6e6; padding: 5px; margin-top: 20px; border-bottom: 1px solid #000; }} table {{ width: 100%; border-collapse: collapse; margin-bottom: 10px; font-size: 9pt; }} th {{ border: 1px solid #000; background-color: #f2f2f2; padding: 4px; text-align: center; font-weight: bold; }} td {{ border: 1px solid #000; padding: 4px; text-align: center; }} .info-grid td {{ border: none; text-align: left; padding: 2px 5px; }} .info-label {{ font-weight: bold; width: 120px; background-color: #f9f9f9; }} .graph-box {{ text-align: center; margin-top: 10px; }} img {{ max-width: 95%; border: 1px solid #ccc; }} .page-break {{ page-break-before: always; }}</style></head><body><div class="header-box"><div class="header-title">CETIS Summary Report</div></div><table class="info-grid"><tr><td class="info-label">Study No.:</td><td>{meta_info.get('study_no','-')}</td><td class="info-label">Test Item:</td><td>{meta_info.get('test_item','-')}</td></tr><tr><td class="info-label">Sponsor:</td><td>{meta_info.get('sponsor','-')}</td><td class="info-label">Date:</td><td>{now}</td></tr><tr><td class="info-label">Batch ID:</td><td>{meta_info.get('batch_id','-')}</td><td class="info-label">Protocol:</td><td>{meta_info.get('protocol','-')}</td></tr><tr><td class="info-label">Analyst:</td><td>{meta_info.get('analyst','-')}</td><td class="info-label">Endpoint:</td><td>{meta_info['endpoint']}</td></tr></table>{comparison_html}<div class="section-title">Point Estimate Summary</div><table><tr><th>Endpoint</th><th>Level</th><th>mg/L</th><th>95% CI</th><th>Method</th></tr>{pe_rows}</table><div class="section-title">Summary of Data</div><table><tr><th>Conc</th><th>N</th><th>Mean</th><th>95% LCL</th><th>95% UCL</th><th>Min</th><th>Max</th><th>Std Err</th><th>CV%</th><th>%Effect</th></tr>{summ_rows}</table>{comp_detail_html}{detail_html}{assumption_html}{anova_html}{graphics_html}</body></html>"""
    return html
...
def get_icpin_values_with_ci(df_resp, endpoint, is_binary=False, total_col=None, response_col=None, n_boot=1000):
    ...
    inhibition_rates = (control_val - y_raw) / control_val if control_val != 0 else np.zeros_like(y_raw)
    ...
    main_results = {l: calc(interpolator, l, control_val) for l in ec_levels}
    ...
    for _ in range(n_boot):
        ...
        y_b = np.maximum.accumulate(np.array(boot_y)[::-1])[::-1]
        try:
            boot_i = interp1d(y_b, x_raw, bounds_error=False, fill_value=np.nan)
            for l in ec_levels:
                v = calc(boot_i, l, y_b[0])
                if not np.isnan(v) and v>0: boot_estimates[l].append(v)
        except: continue
    for l in ec_levels:
        v = main_results[l]
        bs = boot_estimates[l]
        v_s = f"{v:.4f}" if not np.isnan(v) else (f"> {x_raw.max():.4f}" if l>=50 else "n/a")
...
# -----------------------------------------------------------------------------
# [함수 2] 상세 통계 분석 (Zero-Variance & NOEC Fix)
# -----------------------------------------------------------------------------
def perform_detailed_stats(df, endpoint_col, endpoint_name, return_details=False):
    ...
    stats_details = {
        'shapiro_stat': s_stat, 'shapiro_p': s_p, 'shapiro_res': 'Pass' if s_p>0.01 else 'Fail',
        'levene_stat': l_stat, 'levene_p': l_p, 'levene_res': 'Pass' if l_p>0.01 else 'Fail',
        'anova_f': f_stat, 'anova_p': f_p, 'test_name': 'Bonferroni t-test'
    }

    noec = concentrations[0] # Default to control
    loec = "> Max"
    comp_details = []
    ...
    for i, conc in enumerate(concentrations):
        if i == 0: continue # Skip control
        
        # T-test with Zero Variance Handling
        mean_c = np.mean(control_group)
        mean_t = np.mean(groups[conc])
        var_c = np.var(control_group, ddof=1)
        var_t = np.var(groups[conc], ddof=1)
        
        if var_c == 0 and var_t == 0:
            # Both zero variance: if means differ, p=0, else p=1
            p = 0.0 if abs(mean_c - mean_t) > 1e-9 else 1.0
            t_val = 999.9 if p == 0.0 else 0.0
        else:
            t_val, p = stats.ttest_ind(control_group, groups[conc], equal_var=(l_p>0.01))
            # Fallback if ttest returns NaN (should be covered above but safety)
            if np.isnan(p): p = 1.0 
            
        is_sig = p < alpha
        sig_str = "Significant" if is_sig else "Not Significant"
        comp_details.append({'conc': conc, 'stat': f"{t_val:.4f}", 'pval': f"{p:.4e}", 'sig': sig_str, 'alpha': f"{alpha:.4f}"})
        
        if is_sig and not found_loec:
            loec = conc
            # NOEC is the concentration immediately preceding LOEC
            noec = concentrations[i-1]
            found_loec = True
            
    # If no LOEC found, NOEC is the highest concentration
    if not found_loec:
        noec = max(concentrations)
        loec = "> Max"
        
    stats_details['noec'] = noec
    stats_details['loec'] = loec
    ...
    # Return Dictionary to avoid packing error
    return stats_details, summary, comp_details
...
def calculate_ec_lc_range(df, endpoint_col, control_mean, label, is_animal_test=False):
    ...
    try:
        ...
        plot_info = {'type':'probit', 'x': x_smooth, 'y': y_smooth, 'slope': slope, 'intercept': -slope * log_ec50, 'x_original': dose_resp[dose_resp['농도(mg/L)']>0]['농도(mg/L)'], 'y_original': y_obs}
        
    except Exception as e:
        # ICPIN Fallback
        ec_res = {'p': [], 'value': [], 'status': [], '95% CI': []}
        df_icpin = df.copy()
        df_icpin.rename(columns={df.columns[0]: 'Concentration'}, inplace=True)
...
def plot_ec_lc_curve(plot_info, label, ec_lc_results, y_label="Response (%)"):
    ...
    try:
        ...
        if '>' not in str(ec50_val) and 'N' not in str(ec50_val):
            ax.axvline(float(ec50_val), color='green', linestyle='--', label=f'EC50: {ec50_val}')
    except: pass
...
def run_algae_analysis():
    ...
    if 'algae_data_v20' not in st.session_state:
        st.session_state.algae_data_v20 = pd.DataFrame({'농도(mg/L)':[0]*3+[10]*3+[100]*3, '0h':[10000]*9, '24h':[20000]*9, '48h':[80000]*9, '72h':[500000]*9})
    df = st.data_editor(st.session_state.algae_data_v20, num_rows="dynamic")
    if st.button("분석 실행"):
...
def run_daphnia_analysis():
    st.header("🦐 물벼룩")
    if 'd_data_v20' not in st.session_state: st.session_state.d_data_v20 = pd.DataFrame({'농도(mg/L)':[0,6.25,12.5,25,50,100], '총 개체수':[20]*6, '반응 수 (24h)':[0,0,0,0,0,0], '반응 수 (48h)':[0,0,1,5,18,20]})
    df = st.data_editor(st.session_state.d_data_v20, num_rows="dynamic")
    if st.button("분석"):
...
def run_fish_analysis():
    st.header("🐟 어류")
    if 'f_data_v20' not in st.session_state: st.session_state.f_data_v20 = pd.DataFrame({'농도(mg/L)':[0,6.25,12.5,25,50,100], '총 개체수':[10]*6, '반응 수 (24h)':[0]*6, '반응 수 (48h)':[0]*6, '반응 수 (72h)':[0,0,0,2,5,8], '반응 수 (96h)':[0,0,1,4,8,10]})
    df = st.data_editor(st.session_state.f_data_v20, num_rows="dynamic")
    if st.button("분석"):
        for i, t in enumerate(['24h','48h','72h','96h']):
            col = f'반응 수 ({t})'
