...
def generate_full_cetis_report(meta_info, stats_results, ec_results, detail_df, summary_df, dose_resp_fig, raw_resp_fig=None, qq_fig=None, growth_fig=None, report_type="full"):
    ...
    try:
        ...
        for conc, row in pivot_df.iterrows():
            # 소수점 표시 (동물도 입력값이 소수점일 수 있으므로 .4f 허용)
...
def run_algae_analysis():
    ...
    # Key update: v24 (force reset and float init)
    if 'algae_data_v24' not in st.session_state:
        st.session_state.algae_data_v24 = pd.DataFrame({
            '농도(mg/L)':[0.0, 0.0, 0.0, 10.0, 10.0, 10.0, 100.0, 100.0, 100.0], 
            '0h':[10000.0]*9, '24h':[20000.0]*9, '48h':[80000.0]*9, '72h':[500000.0]*9
        })
    if 'analyzed_algae' not in st.session_state: st.session_state.analyzed_algae = False
    
    df = st.data_editor(st.session_state.algae_data_v24, num_rows="dynamic")
    if st.button("분석 실행"):
...
def run_daphnia_analysis():
    st.header("🦐 물벼룩")
    # Float init
    if 'd_data_v24' not in st.session_state: 
        st.session_state.d_data_v24 = pd.DataFrame({
            '농도(mg/L)':[0.0, 6.25, 12.5, 25.0, 50.0, 100.0], 
            '총 개체수':[20.0]*6, 
            '반응 수 (24h)':[0.0]*6, 
            '반응 수 (48h)':[0.0, 0.0, 1.0, 5.0, 18.0, 20.0]
        })
    if 'analyzed_daph' not in st.session_state: st.session_state.analyzed_daph = False
    
    df = st.data_editor(st.session_state.d_data_v24, num_rows="dynamic")
    if st.button("분석"):
    ...
    if st.session_state.analyzed_daph:
        ...
        for t, col in zip(['24h','48h'], ['반응 수 (24h)','반응 수 (48h)']):
            with (t24 if t=='24h' else t48):
                # No NOEC stats call for animals
...
def run_fish_analysis():
    st.header("🐟 어류")
    # Float init
    if 'f_data_v24' not in st.session_state: 
        st.session_state.f_data_v24 = pd.DataFrame({
            '농도(mg/L)':[0.0, 6.25, 12.5, 25.0, 50.0, 100.0], 
            '총 개체수':[10.0]*6, 
            '반응 수 (24h)':[0.0]*6, '반응 수 (48h)':[0.0]*6, 
            '반응 수 (72h)':[0.0, 0.0, 0.0, 2.0, 5.0, 8.0], 
            '반응 수 (96h)':[0.0, 0.0, 1.0, 4.0, 8.0, 10.0]
        })
    if 'analyzed_fish' not in st.session_state: st.session_state.analyzed_fish = False
    
    df = st.data_editor(st.session_state.f_data_v24, num_rows="dynamic")
    if st.button("분석"):
