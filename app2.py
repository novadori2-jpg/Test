...
from scipy.optimize import curve_fit # 필수 추가
import io
import base64
import datetime
...
def calculate_ec_lc_range(df, endpoint_col, control_mean, label, is_animal_test=False):
    ...
    try:
        ...
        plot_info = {'type':'probit', 'x': x_smooth, 'y': y_smooth, # Log Scale X, Probit Y (0-1)
...
def plot_ec_lc_curve(plot_info, label, ec_lc_results, y_label="Response (%)"):
    ...
    ax.scatter(x_orig, y_orig * 100, c='blue', label='Observed', zorder=5)
    
    if plot_info['type'] == 'probit':
        # Probit Curve (Log Scale X)
        # x is log10 scale in plot_info['x']
        x_fit_log = plot_info['x']
        y_fit = plot_info['y'] * 100
        ax.plot(10**x_fit_log, y_fit, 'r-', label='Probit Fit')
        ax.set_xscale('log')
    else:
