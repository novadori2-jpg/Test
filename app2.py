...
1. **분석:** **Linearized Probit (강제 선형화)** 적용으로 무조건 Probit 산출.
...
def calculate_ec_lc_range(df, endpoint_col, control_mean, label, is_animal_test=False):
    ...
    # --- [PROBIT FORCE ENGINE] ---
    # 무조건 Probit 계산을 위해 Linearized Regression 사용
    try:
        if not is_animal_test: raise Exception("Algae uses ICPIN")
        
        # 1. 데이터 준비 (0 제외, 0%->0.1%, 100%->99.9% 보정)
        df_reg = dose_resp[dose_resp['농도(mg/L)'] > 0].copy()
        # 보정: 0.001 ~ 0.999로 클리핑 (Log(0) 및 Inf 방지)
        df_reg['Prop'] = np.clip(df_reg['Inhibition'], 0.001, 0.999)
        
        # X: Log Concentration, Y: Probit (Inverse CDF)
        ...
        # 기울기 양수 강제 (독성이니까 농도 높으면 반응 커져야 함)
        if slope <= 0: 
             slope = 0.1 # Fallback for flat data to avoid crash
        ...
        se_approx = std_err / slope if slope !=0 else 1 # Simplified SE
        ...
        y_smooth = norm.cdf(slope * x_smooth + intercept) # Probit -> Probability
        y_obs = df.groupby('농도(mg/L)')[endpoint_col].sum() / df.groupby('농도(mg/L)')['총 개체수'].sum()
        
        plot_info = {'type':'probit', 'x': x_smooth, 'y': y_smooth, 'slope':slope, 'intercept':intercept, 'x_original': dose_resp['농도(mg/L)'], 'y_original': y_obs.values}
    ...
    if st.button("분석"):
        ...
        for t_l, col in zip(['24h','48h'], ['반응 수 (24h)','반응 수 (48h)']):
            with (t24 if t_l=='24h' else t48):
                ...
                df_detail = df[['농도(mg/L)', col]].copy(); df_detail[col] = df_detail[col]/df['총 개체수']
                df_detail = df_detail.rename(columns={'농도(mg/L)':'Concentration', col: 'Immobility Rate'})
    ...
    if st.button("분석"):
        ...
        for i, t in enumerate(times):
            with tabs[i]:
                ...
                df_detail = df[['농도(mg/L)', col]].copy(); df_detail[col] = df_detail[col]/df['총 개체수']
                df_detail = df_detail.rename(columns={'농도(mg/L)':'Concentration', col: 'Lethality Rate'})
                html = generate_full_cetis_report(meta, None, res, df_detail, summ.rename(columns={'농도(mg/L)':'Concentration', col:'Response'}), fig, None, None, None, "simple")
