...
1. **분석:** **Linearized Probit** (동물), **ICPIN** (조류).
2. **통계:** **Dunnett's Test** (NOEC 산출용) 적용.
""")
...
# -----------------------------------------------------------------------------
# [함수 2] 상세 통계 분석 (Dunnett's Test 적용 + NaN Fix)
# -----------------------------------------------------------------------------
def perform_detailed_stats(df, endpoint_col, endpoint_name, return_details=False):
    ...
    # If scipy >= 1.11
    try:
        # Prepare samples: [treat1, treat2, ...]
        samples = [groups[c] for c in concentrations[1:]]
        res = stats.dunnett(*samples, control=groups[concentrations[0]])
        p_values = res.pvalue
        ...
        for i, conc in enumerate(concentrations[1:]):
            p = p_values[i]
            # NaN Safety for Dunnett? Usually robust, but just in case
            if np.isnan(p):
            ...
            comp_details.append({'conc': conc, 'stat': "-", 'pval': f"{p:.4f}", 'sig': sig_str, 'alpha': "0.05"})
            
            if is_sig and not found_loec:
                loec = conc
                noec = concentrations[i] # One step before
                found_loec = True
        
    except AttributeError:
        # Fallback to Bonferroni if old scipy
        test_name = "Bonferroni t-test (Dunnett unavailable)"
    ...
    if st.button("분석"):
        ...
        for t, col in zip(['24h','48h'], ['반응 수 (24h)','반응 수 (48h)']):
            with (t24 if t=='24h' else t48):
                # NO stats call for animals (NOEC removed)
