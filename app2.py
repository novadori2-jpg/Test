...
def generate_full_cetis_report(meta_info, stats_results, ec_results, raw_df, summary_df, dose_resp_fig, growth_fig=None, report_type="full"):
    ...
    # growth_fig가 None이 아니고 실제 Figure 객체일 때만 변환
    if growth_fig and isinstance(growth_fig, plt.Figure):
    ...
    # Detail Data HTML (Raw Data Pivot)
    detail_html = ""
    try:
...
def run_algae_analysis():
    ...
    if st.button("분석 실행"):
        ...
        with tab1:
            ...
            # !!! 중요: 인자 순서: meta, stats_results, ec_results, raw_df, summary_df, dose_resp_fig, growth_fig !!!
...
def run_animal_analysis(test_name, label):
    ...
    if st.button("상세 분석 실행"):
        ...
        # !!! 중요: 동물 실험은 growth_fig가 없으므로 None 전달 !!!
