...
st.markdown("""
이 앱은 **OECD TG 201, 202, 203** 보고서 요구사항을 충족하며, **사용자 입력 정보**를 반영한 리포트를 생성합니다.
""")
st.divider()

# -----------------------------------------------------------------------------
# [사이드바] 시험 정보 입력 (Report Header Info)
# -----------------------------------------------------------------------------
with st.sidebar:
    st.header("📝 시험 정보 입력")
    ...
    if st.button("상세 분석 실행"):
        ...
        for i, t in enumerate(times):
            with tabs[i]:
                ...
                st.metric(f"{t} {label}50", f"**{val}**", f"CI: {ci}")
