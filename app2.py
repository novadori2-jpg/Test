...
2. **수정:** NOEC 통계 오류 해결, 어류/물벼룩 실행 에러 해결.
""")
...
def generate_full_cetis_report(meta_info, stats_results, ec_results, detail_df, summary_df, dose_resp_fig, raw_resp_fig=None, qq_fig=None, growth_fig=None, report_type="full"):
    ...
    if report_type == "full" and stats_results.get('comp_details'):
        ...
        for row in stats_results['comp_details']:
            ...
            comp_rows += f"<tr style='{style}'><td>{row['conc']}</td><td>{row['stat']}</td><td>{row['pval']}</td><td>{row['sig']}</td></tr>"
        comp_detail_html = f"""<div class="section-title">4. Detailed Comparison Results (vs Control)</div><table><tr><th>Concentration</th><th>T-Stat</th><th>P-Value</th><th>Result</th></tr>{comp_rows}</table>"""
...
# -----------------------------------------------------------------------------
# [함수 2] 상세 통계 분석 (NOEC Fix: NaN P-value)
# -----------------------------------------------------------------------------
def perform_detailed_stats(df, endpoint_col, endpoint_name, return_details=False):
    ...
    if num_groups >= 2:
        ...
        for conc in concentrations[1:]:
            ...
            # *** FIX: P-value is NaN when variance is 0 ***
            if np.isnan(p):
                # Means different? -> Significant
...
def calculate_ec_lc_range(df, endpoint_col, control_mean, label, is_animal_test=False):
    ...
    try:
        ...
        ec_res = {'p': [], 'value': [], 'status': [], '95% CI': []}
        df_icpin = df.copy()
        # *** FIX: No Rename inside fallback (reuse logic) ***
...
def run_algae_analysis():
    ...
    if 'algae_data_v17' not in st.session_state:
        st.session_state.algae_data_v17 = pd.DataFrame({'농도(mg/L)':[0]*3+[10]*3+[100]*3, '0h':[10000]*9, '24h':[20000]*9, '48h':[80000]*9, '72h':[500000]*9})
    df = st.data_editor(st.session_state.algae_data_v17, num_rows="dynamic")
    if st.button("분석 실행"):
        ...
        with tab1:
            ...
            # *** FIX: Add comp_details to stats_res ***
