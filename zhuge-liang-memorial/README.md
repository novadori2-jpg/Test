...
휴대폰용 "Remote Control" 푸시 알림은 이 세션 환경(claude.ai 웹 기반)에서는
신뢰할 수 없어(조용히 전달되지 않는 경우가 있음) 이메일 발송으로 대체했습니다.

- 매일 오전 6시 30분(한국시간)경, 그날의 상소문 전체가 두루마리와 같은 스타일의
  HTML 이메일로 **lej@iai-lab.co.kr** 로 발송됩니다.
- 이메일 하단의 "두루마리로 보기" 버튼을 누르면 지난 상소문 목록이 있는
  본 페이지로 바로 이동합니다.
- 수신 주소를 바꾸고 싶으시면 Claude Code에게 "와룡상소 이메일 수신 주소를
  OOO로 바꿔줘"라고 요청하시면 됩니다.
